...
class StorageProvider with ChangeNotifier {
  ...
  // Fetch all stored items available in marketplace
  Future<void> fetchStoredItems() async {
    _setLoading(true);
    _clearError();
    
    try {
      final items = await _storageService.getStoredItems();
      _storedItems = items;
      _setLoading(false);
      notifyListeners();
    } catch (e) {
  ...
  // Fetch soon-to-expire items for vendor dashboard
  Future<void> fetchExpiringItems() async {
    _setLoading(true);
    _clearError();
    
    try {
      // If a facility is selected, use that facility ID
      if (_selectedFacility != null) {
        _expiringItems = await _storageService.getExpiringItems(_selectedFacility!.id);
      } else {
        // Otherwise, get all expiring items
        _expiringItems = await _storageService.getExpiringItems();
      }
