...
class StorageService {
  ...
  Future<List<StoredItem>> getExpiringItems([String? facilityId, int daysThreshold = 7]) async {
    try {
      if (facilityId != null) {
        // Get items from a specific facility
      ...
      } else if (AppConstants.isDemoMode) {
        // For demo, filter all marketplace items to show those expiring soon
        final allItems = await getStoredItems();
        final now = DateTime.now();
        
        return allItems.where((item) {
          final daysRemaining = item.expiryDate.difference(now).inDays;
          return daysRemaining >= 0 && daysRemaining <= daysThreshold;
        }).toList();
      } else {
        // In a real implementation, we would make an API call here for global expiring items
        final response = await http.get(
          Uri.parse('$_baseUrl/items/expiring?days=$daysThreshold'),
          headers: await _getHeaders(),
        ).timeout(Duration(seconds: AppConstants.apiTimeoutSeconds));
        
        if (response.statusCode == 200) {
          final List<dynamic> data = json.decode(response.body);
          return data.map((json) => StoredItem.fromMap(json)).toList();
        } else {
          throw Exception('Failed to load expiring items: ${response.statusCode}');
        }
      }
    } catch (e) {
      rethrow;
  ...
  // Get all stored items available for marketplace purchase
  Future<List<StoredItem>> getStoredItems() async {
    try {
      if (AppConstants.isDemoMode) {
        // Return demo stored items for MVP
        await Future.delayed(const Duration(milliseconds: 800)); // Simulate network delay
        
        // Create demo stored items
        final List<StoredItem> demoItems = [
          StoredItem(
            id: 'si1',
            productId: 'p1',
            ownerId: 'vendor1',
            ...
            category: 'Fruits',
            quantity: 120.0,
            storedDate: DateTime.now().subtract(const Duration(days: 10)),
          ...
          StoredItem(
            ...
            category: 'Seafood',
            quantity: 50.0,
            storedDate: DateTime.now().subtract(const Duration(days: 3)),
          ...
          StoredItem(
            ...
            category: 'Dairy',
            quantity: 200.0,
            storedDate: DateTime.now().subtract(const Duration(days: 5)),
          ...
          StoredItem(
            ...
            productId: 'p4',
            ownerId: 'vendor1',
            productName: 'Cheese Assortment',
            category: 'Dairy',
            quantity: 30.0,
            storedDate: DateTime.now().subtract(const Duration(days: 8)),
            expiryDate: DateTime.now().add(const Duration(days: 30)),
            bookingType: StorageBookingType.monthly,
            status: BookingStatus.active,
            spaceOccupied: 0.2,
          ...
          StoredItem(
            ...
            category: 'Meat',
            quantity: 80.0,
            storedDate: DateTime.now().subtract(const Duration(days: 2)),
      ...
      final response = await http.get(
        Uri.parse('$_baseUrl/items/marketplace'),
        headers: await _getHeaders(),
      ).timeout(Duration(seconds: AppConstants.apiTimeoutSeconds));
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load stored items: ${response.statusCode}');
      }
    } catch (e) {
      rethrow;
    }
